...
function new_key_vault_rest_client { param(
...
function new_key_vault_rest_client_with_adal_user_credential { param(
	[ValidateNotNullOrEmpty()]$name=$(throw "name is mandatory"),
	[ValidateNotNullOrEmpty()]$aadTenantId = $(throw "aadTenantId is mandatory"),
	[ValidateNotNullOrEmpty()][Security.SecureString]$loginHint = $(throw "loginHint is mandatory"),
