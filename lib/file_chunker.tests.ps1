...
Describe "calculate number of blocks" {
	$fileChunker = new_file_chunker
	Context "when it is less than 4mb" {
		It "returns 1" {
			$result = $fileChunker._calculateNumberOfBlocks(($script:four_mb -1))
			$result | should equal 1
		}
	}
	Context "when it equals 4mb" {
		It "returns 1" {
			$result = $fileChunker._calculateNumberOfBlocks($script:four_mb)
			$result | should equal 1
		}
	}
	...
	Context "when it equals 8mb" {
		It "returns 2" {
			$result = $fileChunker._calculateNumberOfBlocks(($script:four_mb * 2))
			$result | should equal 2
		}
	}
	...
	Context "when it is between 8mb and 12mb" {
		It "returns  3" {
			$result = $fileChunker._calculateNumberOfBlocks((($script:four_mb * 2) + 1))
	...
	Context "when it equals 12mb" {
		It "returns 3" {
			$result = $fileChunker._calculateNumberOfBlocks(($script:four_mb * 3))
			$result | should equal 3
		}
	}
}
...
Describe "calculate block" {
	...
	Context "when the stream is at its start" {
		It "returns the file length when it is less than 4mb" {
			...
			$result = $fileChunker._calculateBuffer($file)
