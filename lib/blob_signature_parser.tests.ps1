...
Describe "create signature" {
	...
	It "capitalizes the verb" {
		$params = @{ Options = @{ Verb = "verb1"; Content = @{ Length = 5 } }; CanonicalizedHeaders = "headers 1"; CanonicalizedResources = "resource 1"; }
		$parser.execute($params)
		$params.Signature | should be "VERB1`n`n`n5`n`n`n`n`n`n`n`n`nheaders 1`nresource 1"
	}
